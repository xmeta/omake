...
\subsection{Handling new languages}
...
\subsubsection{Defining a default compilation rule}
...
a lazy variable (see \href{omake-language-examples.html#lazy}{Lazy Applications}).  In case there
...
\subsubsection{Defining a rule for linking}
...
Now, returning to the animal world, to compute the dependencies of \verb+neko.woof+, we should scan
\verb+neko.cat+, line-by-line, looking for lines of the form \verb+open <name>+.  We could do this
by writing a program, but it is easy enough to do it in \verb+omake+ itself.  We can use the builtin
\verb+awk+ function to scan the source file.  One slight complication is that the dependencies
depend on the \verb+INCLUDE+ path.  We'll use the \verb+find-in-path+ function to find them.  Here
we go.
...
\subsection{Collapsing the hierarchy, .SUBDIRS bodies}
...
\subsubsection{Using glob patterns}
...
solutions is to use one of the directory-listing functions, like \verb+glob+ or \verb+ls+.  The
\verb+glob+ function takes a shell pattern, and returns an array of file with matching filenames in
...
\subsubsection{Simplified sub-configurations}
...
\subsubsection{Computing the subdirectory list}
