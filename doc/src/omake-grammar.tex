...
discussed in the section on dollar modifiers~\ref{section:dollar}.
...
\textbf{Conditionals} (see the section on conditionals~\ref{section:conditionals}).  The \verb+if+ command
...
\textbf{matching} (see the section on matching~\ref{section:match}).  The \verb+switch+ and
...
\textbf{Exceptions} (see the \verb+try+ function~\ref{fun:try}).  The \verb+try+ command
...
\textbf{section} (see the \verb+section+ description~\ref{section:section}).  The \verb+section+ command
...
\textbf{include, open} (see the \verb+include+ description~\ref{section:include}).  The \verb+include+ command
...
\textbf{return} (see the description of functions~\ref{section:functions}).  The \verb+return+ command
...
\textbf{value} (see the description of functions~\ref{section:functions}).  The \verb+value+ command is an identity.
...
\textbf{export} (see the section on scoping~\ref{section:section}).  The \verb+export+ command exports
...
\textbf{while} (see the \verb+while+ description~\ref{fun:while}).  The \verb+while+ command introduces a \verb+while+ loop.
...
\textbf{class, extends} (see the section on objects~\ref{section:objects}).  The \verb+class+ command
...
See the section on variables~\ref{section:variables}.  The simplest variable definition has the
...
See the section on functions~\ref{section:functions}.  A function-application statement is specified as a
...
See the section on objects~\ref{section:objects}.  Objects are defined as an identifier with a
...
See the chapter on rules~\ref{chapter:rules}.  A rule has the following parts.
\begin{enumerate}
...
See the chapter on shell commands~\ref{chapter:shell}.
