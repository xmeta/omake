...
This example uses the quotation \verb+$""...""+~\ref{section:quotes} to quote the text being
...
\verb+file+ function~\ref{fun:file}).  They are then translated to strings for the command line.
This can cause some unexpected behavior.  In the following example, the \verb+absname+
function~\ref{fun:absname} is the absolute pathname for the file \verb+a+, but the rule still prints
...
use the \verb+--absname+~\ref{option:--absname} option.  If you call \verb+omake+ with the
...
can use the \verb+OMakeFlags+ function~\ref{fun:OMakeFlags} to control how it is applied.
