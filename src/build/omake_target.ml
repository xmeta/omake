...
module type NodeOps =
sig
   val cache_exists : Omake_cache.t -> Node.t -> bool
   val explicit_exists : venv -> Node.t -> bool
end

module MakeTargetOps (Arg : NodeOps) =
struct
   open Arg
   ...
   let target_exists_or_is_phony_or_is_explicit cache venv target =
      if debug debug_implicit then
         eprintf "target_exists_or_is_phony_or_is_explicit: %a: %b, %b@." (**)
            ...
            (explicit_exists venv target);
      target_exists_or_is_phony cache target || explicit_exists venv target
...
module NormalArg =
struct
   let cache_exists = Omake_cache.exists
   let explicit_exists = venv_explicit_exists
end

module StrictArg =
struct
   let cache_exists = Omake_cache.exists_realname
   let explicit_exists = venv_explicit_exists_strict
end

module TargetOps = MakeTargetOps (NormalArg);;
module StrictOps = MakeTargetOps (StrictArg);;

let venv_find_buildable_implicit_rule = TargetOps.venv_find_buildable_implicit_rule
let target_is_buildable_proper = TargetOps.target_is_buildable_proper
let target_is_buildable = TargetOps.target_is_buildable

let target_is_buildable_strict = StrictOps.target_is_buildable
let target_is_buildable_proper_strict = StrictOps.target_is_buildable_proper
